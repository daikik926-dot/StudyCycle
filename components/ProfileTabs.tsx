    ...
    <section className="mt-6 rounded-lg border border-slate-200 bg-white shadow-sm">
      ...
      <div className="divide-y divide-slate-200">
        {shownBooks.length === 0 ? (
          ...
          shownBooks.map((book) => (
            <Link key={book.id} href={`/textbooks/${book.id}`} className="flex items-center gap-3 p-4 transition hover:bg-slate-50 sm:p-5">
              <div className="grid h-12 w-12 shrink-0 place-items-center rounded-lg bg-blue-50 text-[#0056b3]">
                {activeTab === "selling" ? <BookOpen className="h-5 w-5" /> : <History className="h-5 w-5" />}
              </div>
              <div className="min-w-0 flex-1">
                <p className="truncate text-sm font-black text-slate-950">{book.title}</p>
                <p className="mt-1 truncate text-xs font-bold text-slate-500">{[book.course, book.professor].filter(Boolean).join(" / ")}</p>
              </div>
              <div className="text-right">
                <p className="text-sm font-black text-[#0056b3]">¥{book.price.toLocaleString()}</p>
                <p className="mt-1 text-xs font-black text-[#28a745]">{STATUS_LABEL[book.status] ?? book.status}</p>
              </div>
            </Link>
          ))
        )}
