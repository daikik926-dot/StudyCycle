  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      id: "system-1",
      sender_type: "system",
      content: "安全のため、初回は定型文のみ送信できます。学術目的が分かる内容から取引を始めてください。",
    ...
    <div className="grid gap-4 lg:grid-cols-[1fr_18rem]">
      <section className="overflow-hidden rounded-lg border border-slate-200 bg-white shadow-sm">
        ...
        <div className="border-t border-slate-200 bg-white p-4">
          {error && (
            <div className="mb-3 rounded-lg bg-red-50 px-4 py-3 text-sm font-bold text-red-700">
              {error}
            </div>
          )}
          ...
          <div className="mt-4 space-y-2">
            {templates.map((template) => (
              ...
              >
                <span>{template}</span>
                {sending ? (
                  <Loader2 className="h-4 w-4 shrink-0 animate-spin text-[#0056b3]" aria-hidden="true" />
                ) : (
      ...
      <aside className="space-y-4">
        ...
        <section className="rounded-lg border border-blue-100 bg-blue-50 p-4">
          ...
          <p className="mt-2 text-sm font-black leading-6 text-slate-700">
            図書館入口・大学生協など、不特定多数が集まる公共の場所を推奨します。
          </p>
        </section>
      </aside>
    </div>
